...
let rec map_all ret_type ret_dest color_map spilled_vars spilled_vars_type restored_vars e =
    ...
    match e with
        | Ans (exp, info) ->
                try_eval info restored_vars (fun x->x) false @@ fun restored_vars restore call_guarded ->
                    let return_reg, let_e, restored_vars = map_exp color_map spilled_vars spilled_vars_type restored_vars call_guarded exp ret_dest ret_type
                    in
        ...
        | Let (((Operand.ID id) as op, op_typ), let_exp, body_st, info) when S.mem id spilled_vars ->
                try_eval info restored_vars (fun x->x) false @@ fun restored_vars restore call_guarded ->
                    let let_reg, let_e, restored_vars = map_exp color_map spilled_vars spilled_vars_type restored_vars call_guarded let_exp op op_typ
                    in
                    ...
                    in
                        AsmReg.Let ((let_reg, op_typ), let_e,
                            AsmReg.Let((reg_dump, Type.Unit (Type.get_info op_typ)), AsmReg.Save(op_reg, id), restore body_e, info),
                            info
                        ), restored_vars
        ...
        | Let ((op, op_typ), let_exp, body_st, info) ->
                try_eval info restored_vars (fun x -> x) false @@ fun restored_vars restore call_guarded ->
                    let op_reg, let_e, restored_vars = map_exp color_map spilled_vars spilled_vars_type restored_vars call_guarded let_exp op op_type
                    in
                    let restored_vars = S.filter (fun var -> M.find var color_map != op_reg) restored_vars
                    in
                    let body_e, restored_vars = map_all ret_type ret_dest color_map spilled_vars spilled_vars_type restored_vars body_st
                    in
                        AsmReg.Let((op_reg, op_typ), let_e, restore body_e, info), restored_vars
and
(*map expression e with return operand and type as (ret_op, ret_type)
 * return triple of (ret_op assigned register, expression e after allocated, list of variables restored
 * Raise Spill id if id is a var to be spilled but not member of restored_vars
 * Exception handler will wrap returned expression by a restoration.
 * Ex.: e = let y = Add (a , b)
 * with spilled_vars = {a, c, d}, restored_vars = {c}
 * map_exp raises Spill a
 * -> jump to try_eval, Spill exception handler
 * try_eval will evaluate map_exp again but with restored_vars = {a, c}.
 * this time map_exp won't raise Spill with a, then returns new_e (assumption, because maybe there still is another Spill)
 * back to try_eval: try_eval wrap new_e by let (find_reg a) = restore(a) in new_e
 *)
map_exp color_map spilled_vars spilled_vars_type restored_vars call_guarded e ret_op ret_type =
    let find_color_opt x e = match (x, e) with
    ...
    in
    (*force find register for ret*)
    find_color_opt true ret_op,
    match e with
