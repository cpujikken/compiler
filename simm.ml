...
and generate' env = function (* 各命令の即値最適化 (caml2html: simm13_gprime) *)
  | Add(x, ID y) when M.mem y env && M.find y env < Common.addi_imm_limit -> Addi(x, Constant (M.find y env))
  | Add(ID x, y) when M.mem x env && M.find x env < Common.addi_imm_limit -> Addi(y, Constant (M.find x env))
  | Add(ID x, ID y) when M.mem x env && M.mem y env && Pervasives.abs (M.find x env + M.find y env) < Common.movei_imm_limit
    -> MoveImm(Constant (M.find y env + M.find x env))
  | ShiftLeft(ID x, ID y) when M.mem x env && M.mem y env && Pervasives.abs (M.find x env lsl M.find y env) < Common.movei_imm_limit
    -> MoveImm(Constant (M.find y env lsl+ M.find x env))
  | ShiftRight(ID x, ID y) when M.mem x env && M.mem y env && Pervasives.abs (M.find x env lsr M.find y env) < Common.movei_imm_limit
    -> MoveImm(Constant (M.find y env lsr M.find x env))
  | Div(ID x, ID y) when M.mem x env && M.mem y env
    -> MoveImm(Constant (M.find y env / M.find x env))
  | Mul(ID x, ID y) when M.mem x env && M.mem y env && Pervasives.abs (M.find x env * M.find y env) < Common.movei_imm_limit
    -> MoveImm(Constant (M.find y env * M.find x env))
  | Sub(x, ID y) when M.mem y env -> Addi(x, Constant (-(M.find y env)))
  | Sub(ID x, ID y) when M.mem x env && M.mem y env && Pervasives.abs (M.find x env - M.find y env) < Common.movei_imm_limit
    -> MoveImm(Constant (M.find y env - M.find x env))
  | Four(ID x) when M.mem x env && Pervasives.abs (M.find x env * 4 ) < Common.movei_imm_limit
    -> MoveImm(Constant (M.find x env * 4))
  | Addi(ID x, Constant i) when M.mem x env && Pervasives.abs (M.find x env  + i ) < Common.movei_imm_limit
    -> MoveImm(Constant (M.find x env  + i))
