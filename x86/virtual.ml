...
let rec g env = function (* 式の仮想マシンコード生成 (caml2html: virtual_g) *)
  ...
  | Closure.IfEq(x, y, e1, e2, info) ->
      ...
      | Type.Bool _ | Type.Int _ -> Ans(IfEq(x, V(y), g env e1, g env e2, info), info)
      | Type.Float _ -> Ans(IfFEq(x, y, g env e1, g env e2, info), info)
      | _ -> failwith "equality supported only for bool, int, and float")
  | Closure.IfLE(x, y, e1, e2, info) ->
      (match M.find x env with
      | Type.Bool _ | Type.Int _-> Ans(IfLE(x, V(y), g env e1, g env e2, info), info)
      | Type.Float _ -> Ans(IfFLE(x, y, g env e1, g env e2, info), info)
  ...
  | Closure.Var(x, info) ->
      (match M.find x env with
      | Type.Unit _ -> Ans(Nop info, info)
      | Type.Float _ -> Ans(FMovD(x, info), info)
  ...
  | Closure.AppDir(x, ys, info) ->
      let (int, float) = separate (List.map (fun y -> (y, M.find y env)) ys) in
      Ans(CallDir(x, int, float, info), info)
  | Closure.Tuple(xs, info) -> (* 組の生成 (caml2html: virtual_tuple) *)
	...
	expand
      ...
      Let((y, Type.Tuple(List.map (fun x -> M.find x env) xs, info)), Mov(reg_hp, info),
	  Let((reg_hp, Type.Int info), Add(reg_hp, C(align offset), info),
	      store, info), info)
  ...
  | Closure.Get(x, y, info) -> (* 配列の読み出し (caml2html: virtual_get) *)
      (match M.find x env with
      | Type.Array(Type.Unit _, _) -> Ans(Nop info, info)
      | Type.Array(Type.Float _, _) -> Ans(LdDF(x, V(y), 8, info), info)
      | Type.Array(_) -> Ans(Ld(x, V(y), 4, info), info)
      | _ -> assert false)
  | Closure.Put(x, y, z, info) ->
      (match M.find x env with
      | Type.Array(Type.Unit _, _) -> Ans(Nop info, info)
      | Type.Array(Type.Float _, _) -> Ans(StDF(z, x, V(y), 8, info), info)
      | Type.Array(_) -> Ans(St(z, x, V(y), 4, info), info)
      | _ -> assert false)
  | Closure.ExtArray(Id.L(x, l_info), info) -> Ans(SetL(Id.L("min_caml_" ^ x, l_info), info), info)
...
let h { Closure.name = (Id.L(x1, x2) as x , t); Closure.args = yts; Closure.formal_fv = zts; Closure.body = e } =
  ...
  let (offset, load) =
    expand
      ...
      (fun z offset load -> fletd(z, LdDF(Id.to_t x, C(offset), 1, info), load, info))
      (fun z t offset load -> Let((z, t), Ld(Id.to_t x, C(offset), 1, info), load, info)) in
  match t with
  | Type.Fun(_, t2, _) ->
